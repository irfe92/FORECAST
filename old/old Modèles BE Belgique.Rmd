...
# variables sélectionnées par regsubset forward MSE: Temp + cosinus + day_length + teff + t1 + t2 + t3 + t5 + t6 + t7 + Date + month + year + day + holidays + lagholidays + wday + quarter + season
...
# Temp + cosinus + sinus + day_length + t1 + t2 + t6 + t7 + month + year + day + holidays + jc + quarter + season 
...
# Temp + cosinus + sinus + day_length + teff + t1 + t2 + month + year + wday + quarter 
...
# variables sélectionnées par regsubset BW BIC: Temp + cosinus + day_length + t1 + t6 + t7 + Date + month + year + day + quarter
...
RL_BE_fin<-lm(Y~ Temp + cosinus + day_length + T00 + t6 + month + day + leadholidays, data=don.train)
...
OLS SANS INTERACTION modèle issu de step
```{r}
...
comparaison MSE RL tot, RL variables significatives, RL step
```{r}
...
# c'est le modèle total qui a la plus petite MSE

```

OLS SANS INTERACTION modèle retenu
```{r}
RL_BE <- lm(Y~.,data=don.train)
...
anova(RL_BE_tot,RL_BE_fin) # la différence semble significative entre tot et tot1
anova(RL_BE_fin,RL_BE_step) # la différence semble significative entre step et tot1
...
RLI_BE_tot<-lm(Y~(Date + cosinus + sinus + day_length + teff + seuil + T00 + month + year + day + weekend + wday + quarter + season + holidays + jc + lagholidays + leadholidays)*Temp ,data=don.train) 
...
```{r}
RLI_BE_fin<-lm( Y~ cosinus + teff + (sinus + teff + month):Temp, data=don.train) 
...
OLS AVEC INTERACTION SIMPLE avec TEMP, modèle issue de step
```{r}
...
```{r}
RLI_BE_step <-lm(formula = Y ~ Date + cosinus + sinus + day_length + teff + T00 + month + year + day + holidays + jc + lagholidays +   leadholidays + Temp + cosinus:Temp + sinus:Temp + day_length:Temp + teff:Temp + month:Temp + jc:Temp + lagholidays:Temp, data = don.train) 
...
OLS AVEC INTERACTION SIMPLE vec TEMP, modèle retenu, résidus
```{r}
...
RLI_BE_multi_tot<-lm(Y~(Date + cosinus + sinus + day_length + month + year + day + weekend + wday + quarter + season + holidays + jc + lagholidays + leadholidays)* (Temp + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + t6 + t7),data=don.train) 
...
OLS AVEC INTERACTION multiple en ne gardant que les variables significatives 
```{r}
# extraction des coefficients significatifs
coef<-RLI_BE_multi_tot_sum$coefficients[,4]
which(coef<0.05)

RLI_BE_multi_fin <- lm(Y~ Temp + teff + Date:(Temp + teff) + sinus:(t4 + t7) + month:(Temp + teff + T00 + t7) + year:(Temp + teff ) + day:teff + holidays:t6 + lagholidays:(Temp + teff + t1 +t3) + leadholidays:(Temp + teff + t6), data=don.train) 
...
OLS AVEC INTERACTION multiple, modele issues de step (tres long)
```{r}
...
OLS AVEC INTERACTION multiple, modele issues de step (tres long même avec parallelisation)
```{r}
...
OLS AVEC INTERACTION multiple modèle issu de step
```{r}
...
OLS AVEC INTERACTION multiple, modèle retenu, résidus
```{r}

# modèle retenu : fin
RLI_BE_multi <- lm(Y~ Temp + teff + Date:(Temp + teff) + sinus:(t4 + t7) + month:(Temp + teff + T00 + t7) + year:(Temp + teff ) + day:teff + holidays:t6 + lagholidays:(Temp + teff + t1 +t3) + leadholidays:(Temp + teff + t6), data=don.train) 

# le graphe des résidus est moins incurvé
plot(RLI_BE_multi)
...
# R² ajusté plus élevé pour RLI 
# residual std error plus faible pour RLI 
...
MSE_BE_RL=c(MSE_RL_BE, MSE_RLI_BE, MSE_RLI_BE_multi)
...
```{r}
which.min(MSE_BE_RL) # c'est RLI qui a la plus petite MSE
...
OLS AVEC INTERACTION  entre Temp (poly degré 2) et les autres variables en ne gardant que les variables significatives 
```{r}
# extraction des coefficients significatifs
coef<-RLI_BE_P2_tot_sum$coefficients[,4]
which(coef<0.05)

RLI_BE_P2_fin <- lm(Y~ Date + cosinus + sinus + day_length + teff + month + year + day + lagholidays + I(poly(Temp,2)):(sinus + day_length +teff + month + lagholidays), data=don.train) 
# RLI_BE_multi_fin_sum<-summary(RLI_BE_multi_fin)
# RLI_BE_multi_fin_sum
# MSE_RLI_BE_multi= mean((Y.test-predict(RLI_BE_multi,don.test))^2)
...
OLS AVEC INTERACTION entre Temp (poly degré 2) et les autres variables modèle issu de step
```{r}
...
```{r}
RLI_BE_P2_step<-lm(formula = Y ~ Date + cosinus + sinus + day_length + teff + T00 + month + year + day + holidays + jc + lagholidays +     leadholidays + I(poly(Temp, 2)) + cosinus:I(poly(Temp, 2)) + sinus:I(poly(Temp, 2)) + day_length:I(poly(Temp, 2)) + teff:I(poly(Temp, 2)) + month:I(poly(Temp, 2)) + jc:I(poly(Temp, 2)) + lagholidays:I(poly(Temp, 2)), data = don.train)
...
```{r}
# R² ajusté plus élevé pour RLI et RLI_P2
# Residual std error le plus faible pour RLI et RLI_P2
...
# On choisit le modele qui a la RMSE la plus petite sur le test set
plot(sqrt(err_poly_BE_HO),ylab="MSE", main='Root MSE Belgique selon le degré de polynome',pch=19,type='b')
...
SP_BE=lm(Y~bs(X,df=4), data=donYX.train) #  1 noeud 
...
GAM_BE_tot=glm(Y~ poly(Temp,6) + Date + cosinus + sinus + day_length + month + year + day + weekend + wday + quarter + season + holidays + jc + lagholidays + leadholidays + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + t6 + t7, data = don.train) 
...
GAM_BE_fin=glm(Y~ poly(Temp,6) + cosinus + sinus + day_length + day + lagholidays, data = don.train) 
...
GAM_BE_step = glm(formula = Y ~ poly(Temp, 6) + cosinus + sinus + day_length + month + year + day + holidays + lagholidays + leadholidays +   teff + t1 + t3 + t7, data = don.train)
...
# modèle retenu : step
GAM_BE <- glm(formula = Y ~ poly(Temp, 6) + cosinus + sinus + day_length + month + year + day + holidays + lagholidays + leadholidays +   teff + t1 + t3 + t7, data = don.train)
...
for (j in 1:nb) {
  ...
  for(i in 1:d) {
    set.seed(1)
    model <- randomForest(Y~., data=train, mtry = 11, ntree=Ntree[i])
...
n_list[which.min(MSE_RF_BE_node)]
# plot(MSE_RF_BE.tree, xlab="ntree", ylab="MSE", main="MSE selon ntree")


barplot(MSE_RF_BE_tree, xlab="ntree", ylab="MSE", ylim = range(MSE_RF_BE_tree) , names = names(MSE_RF_BE_tree) ,main="MSE selon nodesize",las=0) 
...
```{r}
RF_BE_fin<-randomForest(Y~., mtry = 11, ntree=400 ,data=don.train)
...
# les variables les plus importantes sont teff, t3, Temp, month 
...
# prediction
RF_BE_fin_pred<-predict(RF_BE_fin,don.test)
RF_BE_fin_pred_sum<-summary(RF_BE_fin_pred)
# RF_BE_fin_pred_sum

MSE_RF_BE_fin= mean((Y.test-predict(RF_BE_fin,don.test))^2)

# plot des valeurs prédites vs valeurs réelles
ggplot() +
  geom_point(aes(x = don.test$Date, y = Y.test),
             colour = 'red') +
  geom_line(aes(x = don.test$Date, y = RF_BE_fin_pred),
            colour = 'blue') +
