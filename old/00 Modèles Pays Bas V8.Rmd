...
# BASE DONNEES MODELES SANS les variables liées à Temp: TEFF T00 SEUIL
```{r}
...
# suppression des variables zero and near zero variance (ie avec une unique valeur)
nzv<-nearZeroVar(don, saveMetrics = TRUE) # dataframe 4 variables: $freqRatio, $percentUnique, $zerovar, $nzv
# $zerovar, $nzv sont des vecteurs de boolean avec une valeur pour chaque variable
don <- don[,!nzv$nzv]
str(don)


# suppression des variales liées à la température teff (seuil et T00 déjà supprimée par nzv)
var_delete <- c(which(colnames(don)== "teff"))
don<-don[,-var_delete] 
head(don)
...
# grid search pour les paramètres epsilon et cost. Le résultat est une MSE
...
#Regression with SVM
SVR_NL_tot = tuneResult$best.model
