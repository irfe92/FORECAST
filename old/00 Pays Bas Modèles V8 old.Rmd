...
library(forecast)       # plot sympa des résidus checkresiduals et accuracy(RMSE, MAPE, MASE, ne fonctionne pas avec RF, SVR, NN, XGB)
...
formule <- "Y ~ "
for ( i in 1: dim_coef) { formule <- paste ( formule, RL_NL_init_var[i] , "+ ") }
formule
...
# les variables significatives sont Temp, cosinus, day_length, t6, month, day, lagholidays, leadholidays

# résidus
plot(RL_NL_init_sign) # plot residus vs fitted incurvee
...
RLI_NL_fin<-lm( Y~ month + year + day + month:Temp + day:Temp, data=don.train)
...
```{r}
RLI_NL_step <-lm(formula = Y ~ (month + year + day) * Temp, data = don.train)
...
diff_min_max_RLI_NL = (max(MSE_RLI_NL) - min(MSE_RLI_NL))/ min(MSE_RLI_NL)
diff_min_max_RLI_NL # 1.1% d'écart
...
RLI_NL_multi_fin <- lm( Y~ month + year +  day +  Temp +  t1 +  t7 +  month:Temp +  month:t1 + month:t2 +  month:t3 + month:t4 +  month:t5 +  year:t1 +  year:t2 +  year:t5 +  year:t6 +  year:t7, data=don.train) 
...
```{r}
RLI_NL_multi_step <- lm ( Y ~  month + year + day + Temp + t1 + t2 + t3 + t4 + 
    t5 + t6 + t7 + month:Temp + month:t1 + month:t2 + month:t3 + 
    month:t4 + month:t5 + month:t7 + year:t1 + year:t2 + year:t3 + 
    year:t5 + year:t6 + year:t7 + day:Temp, data = don.train) 
...
# les MSE sont assez proches
diff_min_max_RLI_NL_multi = (max(MSE_RLI_NL_multi) - min(MSE_RLI_NL_multi))/ min(MSE_RLI_NL_multi)
...
diff_min_max_RLI_NL_P2 = (max(MSE_RLI_NL_P2) - min(MSE_RLI_NL_P2))/ min(MSE_RLI_NL_P2)
diff_min_max_RLI_NL_P2 # 2% d'écart
...
diff_min_max_RL_NL_all = (max(MSE_NL_RL) - min(MSE_NL_RL))/ min(MSE_NL_RL)
diff_min_max_RL_NL_all # 12.1x
```
...
MSE_POLY_NL_tot= mean((Y.test-predict(POLY_NL,don.test))^2)
...
100*(MSE_GAM_NL_POLY-MSE_GAM_NL_SP)/ MSE_GAM_NL_SP # l'écart est de 0.9%.
...
```{r}
nround_list = seq(from = 10, to = 150 , by = 10)
...
XGBOOST test 2 des nrounds
```{r}
nround_list = seq(from = 80, to = 200 , by = 10)
nb_test= length(nround_list)
mape_test = rep(0, nb_test)
str(mape_test)
