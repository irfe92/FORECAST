---
title: "Synthese Belgique"
author: "NNN"
date: "16 avril 2018"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


```{r}
# COMPARAISON DES MODELES BE

#Load Library
# install.packages("stargaze")
library(stargazer)
library(splines) 
library(randomForest)
library(e1071)
library(ISLR)
library(leaps)
library(questionr)

# sur la base centrée réduite, sans les autres variables méteo
don<-base_BE_F_cr
head(don)

# creation des variables Y et X
don<- rename.variable(don, "Conso", "Y")
head(don)
Y=don$Y 
X=don$Temp
donYX=data.frame(cbind(Y,X))

# Creation de l'echantillon train 2/3 individus et test 1/3
set.seed(1)
dim<-nrow(don)
train=sample(dim,2*dim/3,replace=FALSE)# vecteur d'entier pour la population d'individus en base d'apprentissage
test=model.matrix(Y~.,data=don[-train,])# base de test


```


```{r}


# =======================================================================================================
# MODELES SUR BE 
# =======================================================================================================


# modèle linéaire simple avec que les variables significatives
RL_BE1<-lm(Y~ Temp+cosinus+sinus+day_length+T00+t1+t4+t5+t6+t7+month+day+lagholidays+leadholidays, data=don)
pred_RL_BE1=predict(RL_BE1, newdata=don, se=T)
mse_RL_BE1= mean((Y-predict(RL_BE1,don))^2)

# modèle linéaire simple avec que les variables significatives sélectionnée par step
RL_BE_step<-lm(formula = Y ~ Temp + cosinus + sinus + day_length + T00 + t1 + t4 + t5 + t6 + t7 + month + year + day + holidays + lagholidays + leadholidays, data = don)
mse_RL_BE_step= mean((don$Y-predict(RL_BE_step,don))^2) 


# modèle linéaire avec interaction 
RLI_BE1<-lm(Y~ cosinus + day_length + month + year + day + leadholidays + Temp + (month+year)*Temp,data=don) 
pred_RLI_BE1=predict(RLI_BE1, newdata=don, se=T)
mse_RLI_BE1= mean((Y-predict(RLI_BE1,don))^2)


# poly5 pour BE sur le critère du MSE,issu de la validation hold out train/test et k_folds
poly_BE<- lm(formula=Y~poly(X,5, raw=T), data=don)
pred_poly_BE=predict(poly_BE, newdata=don, se=T)
mse_poly_BE= mean((Y-predict(poly_BE,don))^2)

# modèle spline sur toute la base BE
SP_BE=lm(Y~ns(X,df=3), data=donYX) #  2 noeuds aux quantiles 33% (7.4) et 66% (13.7)
pred_SP_BE=predict(SP_BE, newdata=list(X=donYX$X), se=T)
mse_SP_BE= mean((Y-predict(SP_BE,donYX))^2)

# modèle GAM sur toute la base_BE
gam_BE=lm(Y~poly(Temp,7) + day_length + teff + month + day + holidays + jc + lagholidays + leadholidays,data = don) 
pred_gam_BE=predict(gam_BE, newdata=don, se=T)
mse_gam_BE= mean((Y-predict(gam_BE,don))^2)

# Random Forest sur toute de la base, avec mtry choisi par CV hold out, ntree=500 par défaut
RF_BE<-randomForest(Y~., mtry = 7, data=don)
pred_RF = predict(RF_BE, don)
mse_RF_BE= mean((Y-predict(RF_BE,don))^2)

# SVR sur toute la base BE
svr_BE = svm(Y~.,don)
pred_svr = predict(svr_BE, don)
mse_SVR_BE= mean((Y-predict(svr_BE,don))^2)


# ======================================================================================================
# SYNTHESE DES MODELES objet SVR pas reconnu par stargazer
# ======================================================================================================

stargazer(RL_BE1,RLI_BE1,poly_BE,SP_BE, gam_BE, type='text', flip=TRUE, title="Results", align=TRUE, keep=c("Date"), column.labels = c("RL", "RLI","poly","Spline" ,"GAM"), model.names = TRUE, single.row = TRUE)

# le R² ajusté est le plus élevé pour GAM (0.978) et le plus faible pour Poly et Spline à 0.906
# le residual error est le plus faible pour GAM (0.153) et le plus élevé pour poly et Spline (0.32)
# F-stat est le plus élevé pour Spline (3729) et le plus faible pour RL (1117)


```

```{r}

```

```{r}
# comparaison des MSE entre les modèles RL, RLI, Spline, GAM, SVR
mse_BE=c(mse_RL_BE1, mse_RLI_BE1, mse_poly_BE, mse_SP_BE,mse_gam_BE,mse_RF_BE,mse_svr_BE)

# graphe des MSE
graph<-barplot(mse_BE, xlab="modèles", ylab="MSE", main="MSE des modèles sur la Belgique",las=0)
axis(1, labels=c("Reg.Lin", "RL Inter", "Poly" ,"SPLINE","GAM", "RF", "SVR"), at = graph)

```


```{r}
which.min(mse_BE) # c'est le modèle 6 ie random forest qui présente la plus petite MSE
```


```{r}
# graphe des valeurs prédites selon les modèles
plot(don$Date, don$Y, xlab = "date", ylab="Conso", main="modèles sur la Belgique" )
lines(don$Date,pred_RL_BE1$fit, col="purple")
lines(don$Date,pred_RLI_BE1$fit, col="cyan")
lines(don$Date,pred_poly_BE$fit, col="pink")
lines(don$Date,pred_SP_BE$fit, col="green")
lines(don$Date,pred_gam_BE$fit, col="yellow")
lines(don$Date, pred_svr, col="blue")
lines(don$Date, pred_RF, col="red")
```

```{r}
# graphes avec les 3 meilleurs modèles RF, SVR, GAM
plot(don$Date, don$Y, xlab = "date", ylab="Conso", main="modèles sur la Belgique, GAM(yellow), SVR(blue), RF(red)" )
lines(don$Date,pred_gam_BE$fit, col="yellow")
lines(don$Date, pred_svr, col="blue")
lines(don$Date, pred_RF, col="red")

```



```



