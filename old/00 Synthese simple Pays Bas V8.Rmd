---
title: "Synthese Pays Bas simplifiée"
...
MAPE ne semble pas adapté pour les données centrées réduites, car le dénominateur peut être souvent proche de zero. Il faudrait plutot utiliser le MASE (mean absolute scaled error). Mais accuracy ne reconnaît pas les modèles random forest , SVR, NN, XGB
...
attr(ns(don$Temp,df=SP_NL_df),"knots") # 2 noeuds
...
GAM_NL_SP_df # 3
attr(ns(don$Temp,df=GAM_NL_SP_df),"knots") # 2 noeuds 
...
plot(don.test$Date, Y.test, xlab = "date", ylab="Conso", main="XGB Pays Bas" )
