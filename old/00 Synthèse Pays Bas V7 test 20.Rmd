...
# Creation de l'echantillon train 4/5 et test 1/5
set.seed(1)
dim<-nrow(don)
split=4/5
...
plot(Xtest$Date, Xtest$Y, xlab = "date", ylab="Conso", main="XGB Pays Bas" )
lines(Xtest$Date,pred_XGB_NL, col="tomato") 
...
MSE_NL_r=c(MSE_RLI_NL, MSE_RLI_NL_multi, MSE_GAM_NL_SP, MSE_RF_NL, MSE_XGB_NL)

# graphe des MSE
graph<-barplot(MSE_NL_r, xlab="modèles", ylab="MSE",main="MSE des modèles", las=0)
axis(1, labels=c("RLI","multi", "GAM_SP" ,"RF", "SVR"), at = graph)

# ylim=c(MSE_XGB_NL, MSE_RLI_NL)

```

```{r}
# comparaison des MSE entre les meilleurs modèles
MSE_NL_r=c(MSE_RLI_NL, MSE_RLI_NL_multi, MSE_GAM_NL_SP, MSE_SVR_NL, MSE_NN_NL)

# graphe des MSE
graph<-barplot(MSE_NL_r, xlab="modèles", ylab="MSE",ylim=c(MSE_GAM_NL_SP, MSE_RLI_NL_multi),main="MSE des modèles", las=0)
axis(1, labels=c("RLI","multi", "GAM_SP", "SVR", "NN"), at = graph)
...
stargazer(RL_NL, RLI_NL, RLI_NL_multi, RLI_NL_P2, POLY_NL, SP_NL,GAM_NL_SP, type='text', flip=TRUE, title="Results", align=TRUE, column.labels = c("RL", "RLI", "multi","P2","GAM"), keep = c("Date"), model.names = TRUE, single.row = TRUE)
...
```{r}
# graphes avec les 3 meilleurs modèles XGB, RF, GAM
plot(don.test$Date, don.test$Y, xlab = "date", ylab="Conso", main="modèles XGB(tomato) RF (red) GAM (bisque)" )
lines(don.test$Date, pred_XGB_NL, col="tomato")  
lines(don.test$Date, pred_RF_NL, col="red") 
lines(don.test$Date,pred_GAM_NL_SP$fit, col="bisque")
...
```{r}
plot(Xtest$Date, Xtest$Y, xlab = "date", ylab="Conso", main="modèles XGB (tomato)" )
lines(Xtest$Date,pred_XGB_NL, col="tomato") 
...
plot(don.test$Date, don.test$Y, xlab = "date", ylab="Conso", main="modèles GAM (bisque)" )
lines(don.test$Date,pred_GAM_NL_SP$fit, col="bisque")
