...
RL_BE_fin<-lm(Y~  Temp +  cosinus +  sinus +  day_length +  T00 +  t1 +  t7 +  month +  day, data=don.train)
...
RL_BE_step <- lm(Y ~ Temp + cosinus + sinus + day_length + T00 +  t1 + t4 + t6 + t7 + Date + month + day + holidays + lagholidays, data = don.train)
...
## RLI en ne gardant que les variables signficatives (iterative jusqu'à la stabilité de la formule)
```{r}

RLI_BE_fin<-lm( Y~ cosinus +  teff +  day + sinus:Temp +  day_length:Temp +  teff:Temp + month:Temp, data=don.train) 
...
# modèle retenu
RLI_BE <-RLI_BE_step 
# le graphe des résidus n'a presque plus de structure
plot(RLI_BE)
...
RLI_BE_multi_fin <- lm(Y~ month:Temp +  month:T00 +  month:t7 + lagholidays:t3, data=don.train) 
...
## RLI multi modele issues de step (très très long 23 mn)
```{r}
...
RLI_BE_P2_fin <- lm( Y~ cosinus +  day_length +  teff +  month + day + teff:I(poly(Temp, 2)) +  month:I(poly(Temp, 2)) +  lagholidays:I(poly(Temp, 2)), data=don.train) 
...
GAM_BE_tot=glm(Y~ poly(Temp,which.min(GAM_BE_deg)) + Date + cosinus + sinus + day_length + month + year + day + weekend + wday + quarter + season + holidays + jc + lagholidays + leadholidays + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + t6 + t7, data = don.train) 
...
GAM_BE_fin=glm(Y~ poly(Temp, which.min(GAM_BE_deg)) +  cosinus +  day_length + month + day + leadholidays + T00 + t6, data = don.train) 
...
```{r}
GAM_BE_step <- lm(formula = Y ~ poly(Temp, which.min(GAM_BE_deg)) + cosinus + sinus + day_length + month + year + day + holidays + lagholidays + leadholidays + T00 + t1 + t5 + t6 + t7, data = don.train)
...
# c'est le modèle tot qui est retenu
...
# la valeur par défaut de mtry correspond à la racine carrée du nombre de variables pour classification et p/3 pour la régression
...
RF_BE<-randomForest(Y~., data=don.train, ntree = 500, mtry = 9)
...
```{r}
RF_BE_fin<-randomForest(Y~., mtry = which.min(MSE_RF_BE_mtry), ntree=Ntree[which.min(MSE_RF_BE_tree)], nodesize = RF_BE_node ,data=don.train)
