...
GAM_6P_tot=glm(Y~ poly(Temp,7) + Date + cosinus + sinus + day_length + Pays + month + year + day + weekend + wday + quarter + season + holidays + jc + lagholidays + leadholidays + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + t6 + t7, data = don.train) 

pred_GAM_6P_tot=predict(GAM_6P_tot, newdata=don.test, se=T)
...
GAM modèle retenu en ne gardant que les variables significatives
```{r}

GAM_6P=glm(Y~ poly(Temp,7) + Pays + T00 + t1 +t2 + t3 + t5, data = don.train) 
summary(GAM_6P)
...
GAM modèle step
```{r}
GAM_6P_step <- step(GAM_6P_tot, test="F", trace = FALSE)
GAM_6P_step

GAM_6P_step = glm(formula = Y ~ poly(Temp, 7) + cosinus + sinus + day_length + Pays + year + jc + teff + T00 + t1 + t2 + t3 + t4 + t5 + t6, data = don)
...
GAM comparaison MSE
```{r}
MSE_GAM_6P_tot= mean((Y.test-predict(GAM_6P_tot,don.test))^2)
MSE_GAM_6P= mean((Y.test-predict(GAM_6P,don.test))^2)
MSE_GAM_6P_step= mean((Y.test-predict(GAM_6P_step,don.test))^2)

MSE_GAM_6P_tot=c(MSE_GAM_6P_tot, MSE_GAM_6P, MSE_GAM_6P_step)
which.min(MSE_GAM_6P_tot) # c'est le modèle step qui est retenu
...
lines(don.test$Date, pred_SVR_6P, col="purple")
...
NN CV pour déterminier size et decay
```{r}
...
set.seed(1)
NN_6P_tot <- train(x = don.train[ , colnames(don.train) != "Y"],
...
```{r}
names(NN_6P_tot$finalModel)

```


```{r}
tv=NN_6P_tot$finalModel$tuneValue
str(tv)
