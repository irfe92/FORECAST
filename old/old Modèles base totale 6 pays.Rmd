...
base et variables pour Arbre décision non centrée réduite, sans les autres variables météo
```{r}
...
best_fw_6P_tot=regsubsets(Y~.,data=don[train,],nvmax=p,method='forward')
fw_6P_sum_tot<-summary(best_fw_6P_tot)

# pour chacun des modeles sur le test set, calculer la MSE
mse.fw_6P_tot=rep(NA,p)
for(i in 1:p){
  coefi=coef(best_fw_6P_tot,id=i)
  pred=test[,names(coefi)]%*%coefi
  mse.fw_6P_tot[i]=mean((don$Y[-train]-pred)^2)
}

# on plot les RMSE des p modeles sur le training et sur le test set
# On choisit le modele qui a la RMSE la plus petite sur le test set
plot(sqrt(mse.fw_6P_tot),ylab='Root MSE des p modeles FW', main="Regsubset forward sur base 6 pays",pch=19,type='b')
...
```{r}
which.min(mse.fw_6P_tot) # ici, c'est le modèle à 27 variables
# d'après le graphe MSE, si on veut un modèle sparse, on pourrait prendre 10 car le MSE est quasi stable après
...
coef(best_fw_6P_tot,27)

# variables sélectionnées par regsubset forward: Date + Temp + day_length + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + t6+ Pays + month + year + day + wday + quarter 
...
best_bw_6P_tot=regsubsets(Y~.,data=don[train,],nvmax=p,method='backward')
sum_bw_6P_tot<-summary(best_bw_6P_tot)

# pour chacun des modeles sur le test set, calculer la MSE
mse.bw_6P_tot=rep(NA,p)
for(i in 1:p){
  coefi=coef(best_bw_6P_tot,id=i)
  pred=test[,names(coefi)]%*%coefi
  mse.bw_6P_tot[i]=mean((don$Y[-train]-pred)^2)
}

# on plot les RMSE des p modeles sur le training et sur le test set
# On choisit le modele qui a la RMSE la plus petite sur le test set
plot(sqrt(mse.bw_6P_tot),ylab='Root MSE des p modeles',main="Regsubset backward sur base 6 pays",pch=19,type='b') 
...
x=c(1:p)
y1=sqrt(mse.fw_6P_tot)
y2=sqrt(mse.bw_6P_tot)
...
# summary(best_fw_6P_tot)$rss
# summary(best_bw_6P_tot)$rss
...
# graphe BIC
plot(best_fw_6P_tot, scale="bic", main=" BIC pour Regsubset 6 pays FW") 
...
```{r}
# graphe R² ajusté
plot(best_fw_6P_tot, scale="adjr2", main=" R² Ajuste pour Regsubset 6 pays FW")
...
# graphe Cp
plot(best_fw_6P_tot, scale="Cp", main=" Cp pour Regsubset 6 pays FW")
...
reg.summary<-summary(best_fw_6P_tot)
...
# Afin de nous aider à choisir le modèle à sélectionner, identifier l'emplacement du point maximum / minimum pour chaque critère : $RSS$, $R^2$ ajusté, $C_p$ et $BIC$. Dans chaque cas, afficher les variables sélectionnées.
reg.summary<-summary(best_bw_6P_tot)
...
RL_6P_tot1<-lm(Y~ Temp + day_length + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + Pays, data=don)
...
RL_6P_tot2<-lm(Y~ Temp + day_length + seuil + t1 + t2 + t3 + t4 + t5 + Pays, data=don)
...
RL_6P_tot_step<-lm(formula = Y ~ Temp + day_length + teff + T00 + t1 + t2 + t3 + 
...
# F stat moins élevé que RL_6P_tot2 mais supérieur au modèle total
# residual std error moins élevé que RL_6P_tot1 mais égal modèle total. Mes il est globalement élevé pour les 3 modèles
stargazer(RL_6P_tot,RL_6P_tot2,RL_6P_tot_step,type='text', flip=TRUE, title="Results", keep=c("Date","meteo"), column.labels = c("tot", "tot2","step"))
...
anova(RL_6P_tot2,RL_6P_tot_step) # la différence semble significative entre step et tot2
...
RLI_6P_tot<-lm(Y~(Date + cosinus + sinus + day_length + teff + seuil + T00 + Pays + month + year + day + weekend + wday + quarter + season + holidays + jc + lagholidays + leadholidays)*Temp ,data=don) 
...
RLI_6P_tot1<-lm(Y~ cosinus + day_length + teff + seuil+ Pays + month + day + ( seuil + Pays + month) *Temp,data=don) 
RLI_6P_tot1_sum<-summary(RLI_6P_tot1)
RLI_6P_tot1_sum
...
stargazer(RL_6P_tot,RL_6P_tot2,RL_6P_tot_step,RLI_6P_tot1,type='text', flip=TRUE, title="Results", keep=c("Date"), column.labels = c("tot","tot2","step","RLI"))
...
mse_RL_6P_tot= mean((don$Y-predict(RL_6P_tot,don))^2,na.rm=TRUE) # prediction from a rank-deficient fit may be misleading 

mse_RL_6P_tot2= mean((don$Y-predict(RL_6P_tot2,don))^2, na.rm=TRUE) 

mse_RL_6P_tot_step= mean((don$Y-predict(RL_6P_tot_step,don))^2, na.rm=TRUE) 

mse_RLI_6P_tot= mean((don$Y-predict(RLI_6P_tot,don))^2, na.rm=TRUE)

mse_RLI_6P_tot1= mean((don$Y-predict(RLI_6P_tot1,don))^2, na.rm=TRUE)


mse.BE.RL=c(mse_RL_6P_tot,mse_RL_6P_tot2,mse_RL_6P_tot_step,mse_RLI_6P_tot,mse_RLI_6P_tot1)
which.min(mse.BE.RL) # c'est le 1er RL_6P_tot qui présente la plus petite MSE, avec toutes les variables
...
RLI_6P_P2_tot<-lm(Y~(Date + cosinus + sinus + day_length + teff + seuil + T00 + Pays + month + year + day + weekend + wday + quarter + season + holidays + jc + lagholidays + leadholidays)*I(poly(Temp,2)),data=don) 
...
RLI_6P_P2_tot1<-lm(Y~ cosinus + day_length + teff + seuil + Pays + month + day + lagholidays +(cosinus + seuil + T00 + day_length + Pays + month)*I(poly(Temp,2)), data=don) 
...
# baisse sensible de F stat de 7982 (RLI_6P_tot1) à 6485
stargazer(RLI_6P_tot,RLI_6P_tot1, RLI_6P_P2_tot,RLI_6P_P2_tot1 ,type='text', flip=TRUE, title="Results", keep=c("Date"), column.labels = c("RLI","RLI1", "P2", "P2_tot1"))
...
```{r}
pred_RL_6P_tot1=predict(RL_6P_tot1, newdata=don, se=T)
pred_RLI_6P_tot1=predict(RLI_6P_tot1, newdata=don, se=T)
pred_RLI_6P_P2_tot1=predict(RLI_6P_P2_tot1, newdata=don, se=T)

# graphe des valeurs prédites selon les modèles
plot(don$Date, don$Y, xlab = "date", ylab="Conso", main="modèles RL et RLI 6 PAYS" )
lines(don$Date,pred_RL_6P_tot1$fit, col="blue")
lines(don$Date,pred_RLI_6P_tot1$fit, col="red")
lines(don$Date,pred_RLI_6P_P2_tot1$fit, col="green")

plot(don$Date, don$Y, xlab = "date", ylab="Conso", main="modèle RL sur 6 PAYS" )
lines(don$Date,pred_RL_6P_tot1$fit, col="blue")

# graphe des valeurs prédites selon les modèles
plot(don$Date, don$Y, xlab = "date", ylab="Conso", main="modèle RLI simple sur 6 PAYS" )
lines(don$Date,pred_RLI_6P_tot1$fit, col="red")

# graphe des valeurs prédites selon les modèles
plot(don$Date, don$Y, xlab = "date", ylab="Conso", main="modèles RLI poly2 sur 6 PAYS" )
lines(don$Date,pred_RLI_6P_P2_tot1$fit, col="green")
...
# On choisit le modele qui a la RMSE la plus petite sur le test set
plot(sqrt(mse.poly),ylab="MSE", main='Root MSE 6 pays selon le degré de polynome',pch=19,type='b')
...
poly10_6P_tot<- lm(formula=Y~poly(X,10, raw=T), data=don)
poly10_6P_tot_sum<- summary(poly10_6P_tot)
mse_poly10_6P_tot= mean((Y-predict(poly10_6P_tot,don))^2)
...
POLY_cv.error_6P[i]<-cv.glm(donYX,glm.fit,K=10)$delta[1]
}
...
```{r}
which.min(POLY_cv.error_6P) # ici c'est avec le degré 6 que l'erreur est la plus faible
...
poly6_6P_tot<- lm(formula=Y~poly(X,6, raw=T), data=don)
poly6_6P_tot_sum<- summary(poly6_6P_tot)
mse_poly6_6P_tot= mean((Y-predict(poly6_6P_tot,don))^2)
...
stargazer(RL_6P_tot1,poly10_6P_tot, poly6_6P_tot, type='text', flip=TRUE, title="Results", keep=c("Date"), column.labels = c("RL","poly10", "poly6"))
...
poly_6P_tot<- lm(formula=Y~poly(X,10, raw=T), data=don)
poly_6P_tot_sum<- summary(poly_6P_tot)
mse_poly_6P_tot= mean((Y-predict(poly_6P_tot,don))^2)
...
for(i in 1:DF) {
  ...
  mse_SP_6P_ns[i] <- mean((test$Y-predict(model,test))^2)
  }
...
for(i in 4:DF) {
  ...
  mse_SP_6P_bs[i] <- mean((test$Y-predict(model,test))^2)
  }
...
plot(fit_ns_6P_tot)
mse.SP_6P_ns <- mean((test$Y-predict(fit_ns_6P_tot,test))^2)
# pred_ns_6P_tot=predict(fit_ns_6P_tot, newdata=list(X=test$X), se=T)
...
plot(fit_bs_6P_tot)
mse.SP_6P_bs <- mean((test$Y-predict(fit_bs_6P_tot,test))^2)
# pred_bs_6P_tot=predict(fit_bs_6P_tot, newdata=list(X=test$X), se=T)

diff_6P_bs_ns=mse.SP_6P_bs-mse.SP_6P_ns
diff_6P_bs_ns # mse.SP_6P_bs < mse_bs_6P_ns 
...
SP_6P_tot=lm(Y~bs(X,df=1), data=donYX) #  2 noeuds aux quantiles 33% (7.4) et 66% (13.7)
pred_SP_6P_tot=predict(SP_6P_tot, newdata=list(X=donYX$X), se=T)
mse_SP_6P_tot= mean((Y-predict(SP_6P_tot,donYX))^2)
plot(SP_6P_tot) # graphe des résidus vs fitted a toujours une structure en 3 groupes
...
points (donYX$X,pred_SP_6P_tot$fit, col="blue") 

# graphe de conso vs date
plot(don$Date, don$Y, xlab = "date", ylab="Conso", main="Spline 6 pays, fit (bleu)" )
lines(don$Date,pred_SP_6P_tot$fit, col="blue") 


# SMOOTHING SPLINE
SM_6P_tot=smooth.spline(don$Y,don$X,df=3) # on spécifie df=6 et le lambda est déterminé de sorte à obtenir df=6
SM_6P_tot_cv=smooth.spline(don$Y,don$X,cv=TRUE) # lambda est choisi par cross validation

plot(SM_6P_tot, main="smooth spline") 
plot(SM_6P_tot_cv, main="cv") 

# #comparaison des résultats entre Spline df3, smoothing spline df3 et smoothing spline avec lambda par cv
# stargazer(SP_6P_tot, SM_6P_tot, SM_6P_tot_cv, type='text', flip=TRUE, title="Results", align=TRUE, column.labels = c("SP", "SM", "SMcv"), model.names = TRUE)
```
...
GAM_cv.error_6P_fw=rep(0,d)
for (i in 1:d) { 
  glm.fit=glm(Y~poly(Temp,i) + Date + day_length + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + t6+ Pays + month + year + day + wday + quarter,data = don) 
  GAM_cv.error_6P_fw[i]=cv.glm(don,glm.fit, K=10)$delta[1] # par défaut, K= nombre d'observations donc LOOCV
}

plot(GAM_cv.error_6P_fw, main="GAM 6 pays cv.error selon degré polynome",pch=19,type='b') 

```

GAM détermination du polynome par CV K-fold résultats (avec variables sélectionnées par regsubset forward)
```{r}
which.min(GAM_cv.error_6P_fw) # degré 7
...
GAM_6P_tot_fw=lm(Y~poly(Temp,7) + Date + day_length + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + t6+ Pays + month + year + day + wday + quarter,data = don) 

pred_GAM_6P_tot_fw=predict(GAM_6P_tot_fw, newdata=don, se=T)

summary(GAM_6P_tot_fw)
...
GAM_6P_tot_fw1=lm(Y~poly(Temp,7) + day_length + seuil + T00 + t1 + t2 + t3 + t4 + t5 + t6+ Pays + month + day,data = don) 

summary(GAM_6P_tot_fw1)
...
GAM_6P_tot_fw2=lm(Y~poly(Temp,7) + day_length + seuil + T00 + t1 + t3 + t5 + t6+ Pays + month + day,data = don) 

summary(GAM_6P_tot_fw2)
...
library(boot)
d=15 # degré de spline à tester
GAM_cv.error_6P_bw=rep(0,d)
for (i in 1:d) { 
  glm.fit=glm(Y~poly(Temp,i) + Date + day_length + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + Pays + month + year + day + quarter,data = don) 
  GAM_cv.error_6P_bw[i]=cv.glm(don,glm.fit, K=10)$delta[1] # par défaut, K= nombre d'observations donc LOOCV
}

plot(GAM_cv.error_6P_bw, main="GAM 6 pays BW cv.error selon degré polynome",pch=19,type='b') 

```

GAM détermination du polynome par CV K-fold résultats (avec variables sélectionnées par regsubset backward)
```{r}
which.min(GAM_cv.error_6P_bw)
...
GAM_6P_tot_bw=lm(Y~poly(Temp,7) + Date + day_length + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + Pays + month + year + day + quarter,data = don) 

pred_GAM_6P_tot_bw=predict(GAM_6P_tot_fw, newdata=don, se=T)

summary(GAM_6P_tot_bw)
...
GAM_6P_tot_bw1=lm(Y~poly(Temp,7) + day_length + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + Pays + month + day ,data = don) 

summary(GAM_6P_tot_bw1)
...
GAM_6P_tot_bw2=lm(Y~poly(Temp,7) + day_length + teff + seuil + T00 + t1 + t2 + t3 + t5 + Pays + month + day ,data = don) 

summary(GAM_6P_tot_bw2)
...
GAM_6P_tot_fw2=lm(Y~poly(Temp,7) + day_length + seuil + T00 + t1 + t3 + t5 + t6+ Pays + month + day,data = don.train) 
GAM_6P_tot_bw2=lm(Y~poly(Temp,7) + day_length + teff + seuil + T00 + t1 + t2 + t3 + t5 + Pays + month + day ,data = don.train) 

mse_GAM_6P_tot_fw2= mean((Y.test-predict(GAM_6P_tot_fw2,don.test))^2)
mse_GAM_6P_tot_bw2= mean((Y.test-predict(GAM_6P_tot_bw2,don.test))^2)

diff=mse_GAM_6P_tot_fw2-mse_GAM_6P_tot_bw2
diff # <0 donc c'est fw2 qui est à retenir

which.min(c(mse_GAM_6P_tot_fw2,mse_GAM_6P_tot_bw2)) 
...
```{r}
pred_GAM_6P_tot_fw2=predict(GAM_6P_tot_fw2, newdata=don, se=T)
# graphe des valeurs prédites par GAM sur la Belgique
plot(don$Date, don$Y, xlab = "date", ylab="Conso", main="GAM" )
lines(don$Date,pred_GAM_6P_tot_fw2$fit, col="yellow")
...
RF_6P_tot_train<-randomForest(Y~., data=train, ntree = 500, mtry = 5)
# summary(RF_6P_tot_train)
print(RF_6P_tot_train)
# names(RF_6P_tot) 
...
plot(RF_6P_tot_train$mse, xlab = "nombre d'arbres", ylab = "MSE")
...
for(i in 1:m) {
  ...
  mse.RF_6P[i] <- mean((test$Y-predict(model,test))^2)
  }

# graphe de MSE
plot(mse.RF_6P, xlab="mtry", ylab="MSE", main="MSE selon mtry", type="b")
...
#     mse.RF_6P.tree[i+j-1] <- mean((test$Y-predict(model,test))^2)
#   }  
#   
#   res_ntree[j]=Ntree[which.min(mse.RF_6P.tree)]
#  
# }
# 
# res_ntree
# # plot(mse.RF_6P.tree, xlab="ntree", ylab="MSE", main="MSE selon ntree")
# 
# # graphe des MSE du choix de ntree
# Ntree[which.min(mse.RF_6P.tree)]
# plot(mse.RF_6P.tree, xlab="ntree", ylab="MSE", main="MSE selon ntree")
...
```{r}
RF_6P_tot<-randomForest(Y~., mtry = 10, data=don)

# prediction
RF_6P_tot_pred<-predict(RF_6P_tot,don)
RF_6P_tot_pred_sum<-summary(RF_6P_tot_pred)
# RF_6P_tot_pred_sum

mse_RF_6P_tot= mean((Y-predict(RF_6P_tot,don))^2)

# plot des valeurs prédites vs valeurs réelles
ggplot() +
  geom_point(aes(x = don$Date, y = don$Y),
             colour = 'red') +
  geom_line(aes(x = don$Date, y = RF_6P_tot_pred),
            colour = 'blue') +
...
#Regression with SVM
SVR_6P_tot = svm(Y~.,don)
mse_SVR_6P_tot= mean((Y-predict(SVR_6P_tot,don))^2)

#Predict using SVM regression
pred_svr = predict(SVR_6P_tot, don)
