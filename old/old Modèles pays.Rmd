...
## SELECTION DE VARIABLES AVEC REGSUBSET FW coef MSE
```{r}
...
## SELECTION DE VARIABLES AVEC REGSUBSET FW détails
```{r}
# FW_BE_sum
...
## SELECTION DE VARIABLES AVEC REGSUBSET FW rss
```{r}
...
# selection des variables significatives, en en gardant que les pvalue significatives
RL_BE_fin<-lm(Y~ , data=don.train)
RL_BE_fin_sum<-summary(RL_BE_fin)
# RL_BE_fin_sum

# extraction des coefficients significatifs
coef<-RL_BE_fin_sum$coefficients[,4]
RL_BE_tot_coef <- names(which(coef<0.05))
dim_coef <- length (RL_BE_fin_coef)
formule <- "Y ~ "
for ( i in 1: dim_coef) { formule <- paste ( formule, RL_BE_fin_coef[i] , "+ ") }
formule
...
RL_BE_step<-lm(Y ~ , data = don.train)
...
RLI_BE_fin<-lm( Y~ , data=don.train) 
...
RLI_BE_step <-lm(formula = Y ~ , data = don.train) 
...
RLI_BE_multi_fin <- lm(Y~ ), data=don.train) 
...
## RLI multi modele issues de step (tres long)
```{r}
...
RLI_BE_multi_step <-lm (formula = , data = don.train)
...
## RLI multi modele issues de step (avec parallelisation)
```{r}
...
RLI_BE_P2_fin <- lm( Y~ , data=don.train) 
...
RLI_BE_P2_step<-lm(formula = Y ~ , data = don.train)
...
attr(ns(X,df=which.min(MSE_SP_BE_ns)),"knots")  # 7 noeuds à12.5% 25% 37.5% 50% 62.5% 75% 87.5%
...
attr(bs(X,df=which.min(MSE_SP_BE_bs)+3),"knots") # 3 noeuds
...
GAM_BE_tot=glm(Y~ poly(Temp,which.min(GAM_cv.error_BE)) + Date + cosinus + sinus + day_length + month + year + day + weekend + wday + quarter + season + holidays + jc + lagholidays + leadholidays + teff + seuil + T00 + t1 + t2 + t3 + t4 + t5 + t6 + t7, data = don.train) 
...
GAM_BE_step = glm(formula = Y ~ , data = don.train)
...
for (j in 1:nb) {
  ...
  for(i in 1:d) {
    set.seed(1)
    model <- randomForest(Y~., data=train, mtry = which.min(MSE_RF_BE_mtry), ntree=Ntree[i])
...
for (j in 1:nb) {
  ...
  for(i in 1:d) {
    set.seed(1)
    model <- randomForest(Y~., data=don.train,mtry = which.min(MSE_RF_BE_mtry), ntree=Ntree[which.min(MSE_RF_BE.tree)], nodesize = n_list[i])
...
```{r}
RF_BE_fin<-randomForest(Y~., mtry = which.min(MSE_RF_BE_mtry), ntree=Ntree[which.min(MSE_RF_BE.tree)] , data=don.train)
