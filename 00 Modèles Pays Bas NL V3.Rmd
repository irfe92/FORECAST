...
attr(ns(X,df=which.min(MSE_SP_NL_ns)),"knots")  # 2 noeuds 
...
attr(bs(X,df=which.min(MSE_SP_NL_bs)+3),"knots") # 1 noeud
...
GAM_NL_fin=glm(Y~  poly(Temp, GAM_NL_deg) + Date +  cosinus +  sinus +  day_length +  month +  year +   day +   lagholidays +  leadholidays +  t4 +  t6, data = don.train) 
...
```{r}
RF_NL_fin<-randomForest(Y~., mtry = RF_NL_mtry, ntree=RF_NL_n tree, nodesize = RF_NL_node ,data=don.train)
