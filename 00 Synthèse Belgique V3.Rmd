...
# Temp + cosinus + sinus + day_length + teff + t2 + t6 + month + year + day + holidays + quarter + season 
...
# Temp + cosinus + sinus + day_length + teff + t2 + t5 + t6  + month + year + day + holidays + leadholidays + season
...
RLI_BE=lm(formula = Y ~ Date + cosinus + sinus + day_length + month + year + day + jc + lagholidays + leadholidays + Temp + cosinus:Temp + sinus:Temp + day_length:Temp + month:Temp + year:Temp + jc:Temp + lagholidays:Temp, data = don.train)   
...
# ----- MODELE POLYNOMIAL
# degré du poly sur le critère du MSE,issu de la validation hold out train/test et k_folds
...
# ----- MODELE SPLINES
# modèle spline,CV pour choisir nombre de noeuds et natural splins vs basic splines, par minimisation MSE
...
# ----- MODELE GAM
# degré choisi par CV et minimisation MSE.
# modèle GAM retenu par MSE : total
GAM_BE=lm(formula = Y ~ poly(Temp,GAM_BE_deg) + Date + cosinus + sinus + day_length + month + year + day + weekend + quarter + season + holidays + jc + lagholidays + leadholidays + t1 + t2 + t3 + t4 + t5 + t6 + t7, data = don.train)  
...
# Random Forest , avec mtry (8), ntree (200) et nodesize (1) choisis par CV hold out
...
# détermination par CV de size (6) et decay (0.3)
...
stargazer(RL_BE, RLI_BE, RLI_BE_multi,RLI_BE_P2 ,POLY_BE, SP_BE, GAM_BE, type='text', flip=TRUE, title="Results", align=TRUE, column.labels = c("RL", "RLI", "multi","P2","poly","Spline" ,"GAM"), keep = c("Date"), model.names = TRUE, single.row = TRUE)

# le R² ajusté est le plus élevé pour RLI Multi et le plus faible pour Poly et Spline
# le residual error est le plus faible pour RLI multi  et le plus élevé pour poly et Spline
...
MSE_BE_r=c(MSE_RL_BE, MSE_RLI_BE, MSE_RLI_BE_multi, MSE_GAM_BE, MSE_RF_BE, MSE_SVR_BE)
